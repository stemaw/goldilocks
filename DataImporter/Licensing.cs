...
namespace DataImporter
{
    public class Licensing
    {
        ...
        private IEnumerable<SaleDetail> GetMatchingSales(Car car, IEnumerable<SaleDetail> sales)
        {
            ...
            var exactMatch =
                saleDetails.Where(
                    ...
                    s.Model.ToLower() == car.Model.ToLower() + " " + car.Name.ToLower());
            ...
            {
                fuzzyMatches = Fuzzy.FuzzyMatches(models, car.Model, false);
            }
        ...
        {
            if (model.ToLower().Contains("class") || model.ToLower().Contains("series"))
            {
                ...
                allModels = allModels.Where(m => m.Substring(0, prefixLength) == series && Regex.IsMatch(m.Substring(prefixLength, 1), @"\d+")).ToArray();
            

            }

            return Fuzzy.FuzzyMatches(allModels, model, true);
        }
        ...
        {
            return new CarRepository().AllCars().Where(c => c.Sales == 0);
        }
    }
